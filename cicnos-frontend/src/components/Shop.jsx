...
export default function Shop() {
  return (
    <div className="bg-white relative z-20 rounded-[1.5rem] ">
        ...
        <div className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 lg:gap-x-8">
          {products.map((product) => (
            <a
              key={product.id}
              href={product.href}
              className="group text-sm hover:scale-105 transition-transform duration-500"
            >
              <div className="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-lg bg-gray-100  transition-opacity duration-500 group-hover:opacity-75">
                <img
